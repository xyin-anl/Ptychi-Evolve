...
class VLMEngine:
        ...
        if suffix in ["png", "jpg", "jpeg"]:
            ...
            with Image.open(image_path) as img:
                ...
                if hasattr(img, "n_frames") and img.n_frames > 1:
                    self.log.warning(
                        f"Multi-page TIFF detected ({img.n_frames} pages). Only the first page will be used."
                    )
            ...
            raise ValueError(
                f"Unsupported image type: {suffix}. Supported types: png, jpg, jpeg, tiff, tif"
            )
    ...
    def add_few_shot_example(self, image_path: Path, evaluation: Any):
        ...
        self.log.info(
            f"Added few-shot example from {image_path}. Total examples: {len(self.few_shot_examples)}"
        )
    ...
    def _build_few_shot_messages(self, image_path: Path) -> List[Dict]:
        ...
        for i, example in enumerate(self.few_shot_examples):
            ...
            messages.append(
                {"role": "assistant", "content": json.dumps(eval_data, indent=2)}
            )
    ...
    def evaluate_with_few_shot(self, image_path: Path) -> Dict[str, Any]:
        ...
        if not self.few_shot_examples:
            raise ValueError(
                "No few-shot examples provided. Please add examples first."
            )
    ...
    def _build_description_messages(
        self, image_path: Path, evaluation_description: str
    ) -> List[Dict]:
...
{{
    ...
    def evaluate_with_description(
        ...
        """
        # Build messages for description-based evaluation
        messages = self._build_description_messages(image_path, evaluation_description)
