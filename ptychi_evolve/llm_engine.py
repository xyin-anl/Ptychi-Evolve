...
class LLMEngine:
    ...
    def __init__(
        self, config: Dict[str, Any], verbose: bool = False, debug: bool = False
    ):
        ...
        try:
            ...
            if isinstance(input_content, str):
                ...
                if len(input_content) > 500:
                    self.log.debug_info(
                        f"... ({len(input_content) - 500} more chars)", 2
                    )
            else:
    ...
    def _extract_algorithm_from_text(
        ...
        if "def regularize_llm(" not in code_block:
            ...
            if m:
                ...
                funcs = re.findall(
                    r"^\s*def\s+([a-zA-Z_]\w*)\s*\(", code_block, re.MULTILINE
                )
                if len(funcs) == 1:
                    raise ValueError(
                        f"Expected function 'regularize_llm', found '{found_name}'"
                    )
    ...
    def generate_algorithm(
        ...
        formatted_prompt = prompt.format(
            ...
            aggregated_suggestions=json.dumps(
                context.get("aggregated_suggestions", []), indent=2
            ),
        )
    ...
    def tune_parameters(self, prompt: str, algorithm: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.log.llm(
            f"Tuning parameters for algorithm {algorithm.get('id', 'unknown')}"
        )
    ...
    def crossover_algorithms(
        self, prompt: str, population: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
    ...
    def evolve_algorithms(
        self, prompt: str, population: List[Dict[str, Any]], strategy: str
    ) -> List[Dict[str, Any]]:
    ...
    def correct_algorithm(
        self, prompt: str, algorithm: Dict[str, Any], error: str
    ) -> Dict[str, Any]:
    ...
    def analyze_results(
        self, prompt: str, algorithm: Dict[str, Any], history: DiscoveryHistory
    ) -> Dict[str, Any]:
        ...
        if self.debug:
            ...
            if "techniques" in analysis:
                self.log.debug_info(
                    f"Techniques found ({len(analysis['techniques'])}):", 1
                )
                for tech in analysis["techniques"][:5]:
                    self.log.debug_info(f"- {tech}", 2)
            if "parameters" in analysis:
                self.log.debug_info(
                    f"Parameters extracted: {len(analysis['parameters'])}", 1
                )
