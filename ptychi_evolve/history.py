...
class DiscoveryHistory:
        ...
        if eval_mode == "ground_truth":
            ...
            if self.performance_levels_ground_truth_label_sense == "higher_is_better":
                ...
                sorted_levels = sorted(
                    self.performance_levels_ground_truth.items(),
                    key=lambda x: x[1],
                    reverse=True,
                )
                ...
                sorted_levels = sorted(
                    self.performance_levels_ground_truth.items(), key=lambda x: x[1]
                )
                for level, threshold in sorted_levels:
                    if ground_truth_value <= threshold:
                        return level
            ...
            if (
                "structured_evaluation" in metrics
                ...
                if (
                    ...
                    # Sort thresholds in descending order to check highest first
                    sorted_levels = sorted(
                        self.performance_levels_qualitative.items(),
                        key=lambda x: x[1],
                        reverse=True,
                    )
                    for level, threshold in sorted_levels:
                    ...
                    # Sort thresholds in ascending order to check lowest first
                    sorted_levels = sorted(
                        self.performance_levels_qualitative.items(), key=lambda x: x[1]
                    )
                    for level, threshold in sorted_levels:
                        if qualitative_value <= threshold:
                            return level
    ...
    def _load_existing(self) -> None:
        ...
        try:
            ...
            logging.warning(
                f"Failed to load existing history file '{latest_file}': {e}. Starting fresh."
            )
    ...
    def __init__(self, config: Dict[str, Any], name: str):
        ...
        # Limits from config
        self.compression_threshold = self.history_config.get(
            "compression_threshold", 50
        )
        # Context sizing (configurable)
        self.recent_for_context = self.history_config.get(
            "recent_for_context", N_RECENT_ALGORITHMS
        )
        self.suggestion_pool_last_k_high = self.history_config.get(
            "suggestion_pool_last_k_high", 10
        )
        self.recent_suggestions_last_k = self.history_config.get(
            "recent_suggestions_last_k", 5
        )

        # Compression policy configuration
        self.compression_moderate_share = self.history_config.get(
            "compression_moderate_share", 0.25
        )  # Share of residual budget for moderate algorithms
        self.compression_min_moderate = self.history_config.get(
            "compression_min_moderate", 5
        )  # Minimum number of moderate algorithms to keep
        self.compression_keep_top_n = self.history_config.get(
            "compression_keep_top_n", 3
        )  # Always keep top N performers regardless of classification
        ...
        self.performance_levels_ground_truth_label = analysis_config.get(
        ...
        assert (
            self.performance_levels_qualitative_label != "feedback"
    ...
    def _compress(self) -> None:
        ...
        if moderate_indices:
            ...
            keep_count = min(
                len(moderate_indices),
                max(
                    int(residual_budget * self.compression_moderate_share),
            ...
            if keep_count > 0:
                ...
                best_moderate_indices = [
                    item[0] for item in sorted_moderates[:keep_count]
                ]
            ...
            if poor_indices:
                ...
                keep_count = min(
                    ...
                    max(
                        self.compression_min_moderate,
                        int(
                            self.compression_threshold * self.compression_moderate_share
                        ),
                    ),
                )
    ...
    def _summarize_algorithm(
        self, algorithm: Dict[str, Any], keep_code: bool = False
    ) -> Dict[str, Any]:
    ...
    def get_context(self) -> Dict[str, Any]:
        ...
        context = {
            "recent_algorithms": self.summarize_algorithms(
                self.get_recent(self.recent_for_context)
            ),
            "best_performance": (
                self._summarize_algorithm(self.get_best_algorithm(), keep_code=True)
                if self.get_best_algorithm()
                else {}
            ),
        }
    ...
    def add(self, algorithm: Dict[str, Any]) -> None:
        ...
        if "action" in algorithm:
            action_map = {"generate": "generated", "tune": "tuned", "evolve": "evolved"}
            algorithm["action"] = action_map.get(
    ...
    def save(self, path: Optional[Path] = None) -> None:
        ...
        save_path = (
            path
            or self.save_path
            / f"{self.name}_history_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
    ...
    def export(self) -> Dict[str, Any]:
        ...
        evolution_stats = {
            "mutations": sum(
                ...
                if a.get("action") == "evolved" and a.get("strategy") == "mutation"
            ),
            "crossovers": sum(
                1
                for a in self.algorithms
                if a.get("action") == "evolved" and a.get("strategy") == "crossover"
            ),
            "generated": sum(
                1 for a in self.algorithms if a.get("action") == "generated"
            ),
            "tuned": sum(1 for a in self.algorithms if a.get("action") == "tuned"),
            "evolved": sum(
                1 for a in self.algorithms if a.get("action") == "evolved"
            ),  # Total evolved
        }
