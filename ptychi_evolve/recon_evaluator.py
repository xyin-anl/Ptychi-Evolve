...
def safe_eval_math(expr):
    ...
    try:
        ...
        logging.warning(
            f"safe_eval_math: failed to parse expression '{expr}', passing it through unchanged"
        )
        return expr
...
class ReconEvaluator:
    ...
    def __init__(
        ...
        self.log.eval(
            f"Iterations per evaluation: {self.eval_config.get('iterations', 500)}"
        )
        ...
        if self.eval_mode in ["few_shot", "vision_description"]:
            assert (
                VLMEngine is not None
            ), "VLM engine is not available for {} mode evaluation".format(
                self.eval_mode
            )
    ...
    def _load_base_params(self) -> Dict[str, Any]:
        ...
        params = {
            ...
            "path_to_processed_hdf5_dp": recon_config.get(
                "path_to_processed_hdf5_dp", ""
            ),
            "path_to_processed_hdf5_pos": recon_config.get(
                "path_to_processed_hdf5_pos", ""
            ),
            "transpose_diffraction_patterns": recon_config.get(
                "transpose_diffraction_patterns", False
            ),
            ...
            "init_probe_propagation_distance_mm": recon_config.get(
                "init_probe_propagation_distance_mm", 0.0
            ),
            "orthogonalize_initial_probe": recon_config.get(
                "orthogonalize_initial_probe", False
            ),
            "position_correction": recon_config.get("position_correction", False),
            "position_correction_update_limit": recon_config.get(
                "position_correction_update_limit", 10.0
            ),
            "position_correction_affine_constraint": recon_config.get(
            ...
            "update_object_w_higher_probe_modes": recon_config.get(
            ...
            "batch_selection_scheme": recon_config.get(
                "batch_selection_scheme", "random"
            ),
            ...
            "save_diffraction_patterns": recon_config.get(
    ...
    def _create_function(self, code: str):
        ...
        def _limited_import(name, globals=None, locals=None, fromlist=(), level=0):
            ...
            if name not in ALLOWED_IMPORTS and root not in ALLOWED_IMPORTS:
                raise ImportError(
                    f"Import of '{name}' is not allowed in LLM regularizer."
                )
            ...
            def __getattr__(self, name):
                if self._module is None:
                    try:
                        ...
                        raise ImportError(
                            f"Failed to import {self._module_name} required by algorithm: {e}"
                        )
    ...
    def _compute_phase_metrics(
        self, rec_phase_path: Union[str, Path]
    ) -> Dict[str, float]:
    ...
    def _collect_human_evaluation(
        ...
        self.log.eval(
            "Please review the reconstructed phase image and provide your evaluation."
        )

        self.log.eval(
            "Please evaluate the reconstruction based on the following criteria:"
        )
        ...
        self.log.eval(
            "What aspects of the reconstruction work well? What needs improvement?"
        )
        sys.stdout.flush()
        ...
        self.log.eval(
            "Based on this evaluation, what should the discovery system do next?"
        )
        self.log.eval(
            "Options: generate (new algorithm), tune (parameters), evolve (combine algorithms)"
        )
        sys.stdout.flush()
        suggested_action = (
            input("Suggested action [generate/tune/evolve]: ").lower().strip()
        )
    ...
    def _calculate_metrics_human(
        self, phase_path: str, final_loss: float = None
    ) -> Dict[str, Any]:
    ...
    def _confirm_vlm_evaluation(
        self, vlm_results: Dict[str, Any], image_path: Union[str, Path]
    ) -> Dict[str, Any]:
    ...
    def _calculate_metrics_vlm_few_shot(
        self, phase_path: str, algorithm_info: Dict[str, Any], final_loss: float = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if require_confirmation:
                ...
                confirmed_results = self._confirm_vlm_evaluation(
                    vlm_results, phase_path
                )
    ...
    def _calculate_metrics_vlm_description(
        self, phase_path: str, algorithm_info: Dict[str, Any], final_loss: float = None
    ) -> Dict[str, Any]:
        """Extract metrics using VLM with natural language description."""
        assert (
            "evaluation_description" in self.eval_config
        ), "Evaluation description is not available"
        assert (
            self.eval_config["evaluation_description"] != ""
        ), "Evaluation description is empty"
        assert (
            self.eval_config["evaluation_description"] != "N/A"
        ), "Evaluation description is N/A"
        assert (
            self.eval_config["evaluation_description"] != None
        ), "Evaluation description is None"
        ...
        try:
            ...
            if require_confirmation:
                ...
                self.log.eval(
                    f"VLM Quality Score: {vlm_results.get('quality_score', 'N/A')}/1.0"
                )
                ...
                if confirm.startswith("n"):
                    self.log.prompt("Enter human override:")
                    human_metrics = self._collect_human_evaluation(
                        phase_path, final_loss
                    )
                    metrics["structured_evaluation"] = human_metrics
                    # Extract suggested action if present
                    if "suggested_action" in human_metrics:
    ...
    def evaluate(
        self, algo_id: str, code: str, algorithm_data: Dict[str, Any] = None
        ...
        try:
            ...
            self.log.eval(
                f"Data: {params.get('data_directory')}, Scan: {params.get('scan_num')}"
            )
            ...
            if isinstance(result, tuple):
                if len(result) >= 2:
                    ...
                    raise ValueError(
                        f"pear.ptycho_recon returned unexpected tuple length: {len(result)}"
                    )
            else:
                raise ValueError(
                    f"pear.ptycho_recon returned unexpected type: {type(result)}"
                )
