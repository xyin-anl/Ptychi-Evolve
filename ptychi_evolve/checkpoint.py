...
class CheckpointManager:
    ...
    def save_checkpoint(self, state: Dict[str, Any], force: bool = False) -> bool:
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(
                mode="w", delete=False, dir=self.save_path, suffix=".tmp"
            ) as tmp_file:
                try:
                    ...
                    self.log.error(
                        f"Problematic data structure - stats: {state.get('stats', {})}"
                    )
                    self.log.error(
                        f"Number of algorithms: {len(state.get('history', {}).get('algorithms', []))}"
                    )
                    # Log which algorithms are being dropped to save a minimal checkpoint
                    try:
                        removed_ids = [
                            a.get("id", "unknown")
                            for a in state.get("history", {}).get("algorithms", [])
                        ]
                        if removed_ids:
                            self.log.warning(
                                f"Removed algorithms from checkpoint: {', '.join(map(str, removed_ids))}"
                            )
                    except Exception:
                        pass
                    ...
                    minimal_checkpoint = {
                        ...
                        "discovery_state": {
                            ...
                            "history": {
                                ...
                                "metadata": state.get("history", {}).get(
                                    "metadata", {}
                                ),
                            },
                    ...
                    tmp_path = tmp_file.name
                    self.log.warning(
                        "Saved minimal checkpoint due to serialization error"
                    )
    ...
    def load_checkpoint(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.log.checkpoint(
                f"  Generation: {stats.get('total_generated', 0)}, Timestamp: {time.ctime(checkpoint.get('timestamp', 0))}"
            )
            return state
            ...
            if self.backup_file.exists():
                try:
                    ...
                    stats = state.get("stats", {})
                    self.log.checkpoint(
                        f"✓ Backup checkpoint loaded from {self.backup_file}"
                    )
                    self.log.checkpoint(
                        f"  Generation: {stats.get('total_generated', 0)}, Timestamp: {time.ctime(checkpoint.get('timestamp', 0))}"
                    )
