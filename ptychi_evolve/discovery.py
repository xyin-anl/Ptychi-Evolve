...
class AlgorithmDiscovery:
    ...
    def __init__(
        ...
        example_algorithms: Optional[List[str]] = None,
        verbose: bool = False,
        debug: bool = False,
    ):
        """
        ...
        self.log.init(
            f"Evaluation mode: {self.config.get('evaluation', {}).get('mode', 'unknown')}"
        )
        ...
        self.evaluator = ReconEvaluator(
            self.config, self.llm, verbose=self.verbose, debug=self.debug
        )
        ...
        self.checkpoint_manager = CheckpointManager(
            save_path, self.name, checkpoint_interval
        )
        ...
        self.action_policy = {
            "honor_suggestion_min_level": dsp.get(
        ...
        self.early_stop = {
            ...
            "target_good_plus_excellent": esp.get(
                "target_good_plus_excellent", self.n_target_algorithms
            ),
            "target_any_successful": esp.get(
                "target_any_successful", self.n_target_algorithms
            ),
            "require_min_good_plus_excellent": esp.get(
                "require_min_good_plus_excellent", 5
            ),
            "require_at_least_one_excellent": bool(
                esp.get("require_at_least_one_excellent", True)
            ),
        }

        # Error policy (max consecutive errors before abort)
        self.max_consecutive_errors = int(
            self.config.get("discovery", {}).get("max_consecutive_errors", 5)
        )
    ...
    def _signal_handler(self, signum, frame):
        ...
        try:
            ...
            self.log.discovery(
                f"[DISCOVERY] To resume, run with the same config and name: '{self.name}'"
            )
    ...
    def _validate_config(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if missing_prompts:
            error_msg = (
                f"Missing required prompt templates: {', '.join(missing_prompts)}"
            )
    ...
    def gather_experiment_context(self) -> str:
        ...
        self.log.discovery(
            "Please provide information about your ptychography experiment."
        )
        ...
        self.log.prompt(
            "Enter your description (press Enter on an empty line when done):\n"
        )
    ...
    def _decide_next_action(self) -> str:
        ...
        if self.history.size() > 0:
            ...
            if recent_algos:
                ...
                if "suggested_action" in last_algo.get("metrics", {}):
                    ...
                    perf_level = self.history._classify_performance(
                        last_algo.get("metrics", {})
                    )
                    ...
                    if self._perf_rank(perf_level) >= self._perf_rank(min_level):
                        ...
                        self.log.decision(
                            f"Using evaluator suggestion from {perf_level} algorithm (>= {min_level}): {action}"
                        )
                        return action
                    self.log.decision(
                        f"Ignoring suggestion from {perf_level} (< {min_level})"
                    )
        ...
        # If we have enough excellent algorithms, tune them
            ...
            num_excellent >= self.action_policy["tune_min_excellent"]
            and self.stats["total_generated"]
            % max(1, self.action_policy["tune_every_k"])
            == 0
        ):
            action = "tune"
        # If we have a good population, evolve
        elif (
            total_successful >= self.action_policy["evolve_min_successful"]
            and self.stats["total_generated"]
            % max(1, self.action_policy["evolve_every_k"])
            == 0
        ):
            action = "evolve"
        ...
        self.log.decision(
            f"Action: {action} (excellent: {num_excellent}, good: {num_good}, moderate: {num_moderate})"
        )
    ...
    def _generate_algorithm(self) -> Dict[str, Any]:
        ...
        if self.example_algorithms:
            context += (
                "\n\n"
                + "Example algorithms: "
                + json.dumps(self.example_algorithms, indent=2)
            )
    ...
    def _tune_algorithms(self) -> List[Dict[str, Any]]:
        ...
        self.log.tune(
            f"Starting parameter tuning (attempt #{self.stats['tuning_attempts']})"
        )
        ...
        for i, candidate in enumerate(candidates):
            self.log.tune(
                f"Tuning candidate {i+1}/{len(candidates)}: {candidate['id']}"
            )

            # Tune using LLM
            tuned_algorithm = self.llm.tune_parameters(
                prompt=self.config["prompts"]["parameter_tuning"], algorithm=candidate
            )
            if tuned_algorithm:
                # Ensure algorithm has an ID
    ...
    def _correct_algorithm(
        self, algorithm: Dict[str, Any], error: str
        ...
        for attempt in range(max_attempts):
            ...
            if error:
                self.log.correct(
                    f"Attempting correction {attempt + 1}/{self.max_correction_attempts}"
                )
                ...
                try:
                    ...
                    if corrected:
                        ...
                        corrected["original_id"] = algorithm.get(
                            "original_id", algorithm["id"]
                        )

                        # Try evaluating the corrected version
                        success, metrics, error = self.evaluator.evaluate(
                            corrected["id"], corrected["code"], corrected
                        )
    ...
    def _process_algorithm(self, algorithm: Dict[str, Any]) -> None:
        ...
        if "id" not in algorithm:
            algorithm["id"] = uuid.uuid4().hex
        self.log.process(
            f"Algorithm {algorithm['id']} (action: {algorithm.get('action', 'unknown')})"
        )

        # Evaluate the algorithm
        success, metrics, error = self.evaluator.evaluate(
            algorithm["id"], algorithm["code"], algorithm
        )
        ...
        if algorithm.get("success", False):
            ...
            self.stats["failed"] += 1
            self.log.process(
                f"Failed: {algorithm.get('error', 'Unknown')[:100]}...", success=False
            )
    ...
    def _should_stop_early(self) -> bool:
        ...
        if len(excellent_algos) >= self.early_stop["target_excellent"]:
            self.log.info(
                f"Found {len(excellent_algos)} excellent algorithms. Stopping early."
            )
            return True
        ...
        good_algos = self.history.get_by_performance("good")
        if (
            len(excellent_algos) + len(good_algos)
            >= self.early_stop["target_good_plus_excellent"]
        ):
            self.log.info(
                f"Found {len(excellent_algos)} excellent and {len(good_algos)} good algorithms. Stopping early."
            )
            return True
        ...
        if total_successful >= self.early_stop["target_any_successful"] and (
            (
                len(excellent_algos) >= 1
                if self.early_stop["require_at_least_one_excellent"]
                else True
            )
            or (
                len(excellent_algos) + len(good_algos)
                >= self.early_stop["require_min_good_plus_excellent"]
            )
        ):
            self.log.info(
                f"Found {total_successful} successful algorithms ({len(excellent_algos)} excellent, {len(good_algos)} good, {len(moderate_algos)} moderate). Stopping early."
            )
            return True
    ...
    def _generate_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "statistics": {
                ...
                / max(1, self.stats["successful"] + self.stats["failed"]),
    ...
    def export_results(self, output_dir: str) -> None:
        ...
        try:
            ...
            summary_file.write_text(
                json.dumps(self._sanitize_for_json(summary), indent=2, allow_nan=False)
            )
...
'''
    ...
    def _load_checkpoint(self) -> None:
        ...
        if checkpoint:
            ...
            if isinstance(history_data, dict) and "algorithms" in history_data:
                ...
                self.log.warning(
                    "Legacy checkpoint format detected, starting with empty history"
                )
    ...
    def _save_checkpoint(self, force: bool = False) -> bool:
        ...
        if (
            ...
            # Debug logging
            self.log.debug(
                f"Attempting checkpoint save (force={force}, total_generated={self.stats['total_generated']})"
            )

            # Export history to a serializable format
            # Sanitize algorithms to ensure JSON serialization works
            sanitized_algorithms = [
                self._sanitize_for_json(algo) for algo in self.history.algorithms
            ]
            ...
            history_data = {
                ...
                "metadata": {
                    ...
                    "compression_threshold": getattr(
                        self.history, "compression_threshold", 50
                    ),  # Default if missing
                },
            }
            ...
            self.log.debug(
                f"Checkpoint data: {len(history_data['algorithms'])} algorithms, stats: {self.stats}"
            )
    ...
    def discover(self, max_iterations: Optional[int] = None) -> Dict[str, Any]:
        ...
        if self.verbose:
            ...
            self.log.discovery(
                f"Target: {self.n_target_algorithms} high-quality algorithms"
            )
            self.log.discovery(
                f"Experiment: {self.experiment_context[:100]}..."
                if self.experiment_context
                else "No experiment context"
            )
        ...
        while (
            self.stats["total_generated"] < max_attempts
            ...
            try:
                ...
                if action == "tune":
                    ...
                    if not algorithms:
                        self.log.info(
                            "No algorithms to tune, falling back to generation"
                        )
                    ...
                    if not algorithms:
                        self.log.info(
                            "No algorithms to evolve, falling back to generation"
                        )
                        algorithms = [self._generate_algorithm()]
                else:  # generate
                ...
                if self.stats["total_generated"] + len(algorithms) > max_attempts:
                    ...
                    self.log.info(
                        f"Trimming batch to {remaining} algorithms to stay within max_attempts limit"
                    )
                ...
                self.log.discovery(
                    "[DISCOVERY] Session interrupted! Progress has been saved."
                )
                self.log.discovery(
                    f"[DISCOVERY] To resume, run with the same config and name: '{self.name}'"
                )
                self.log.discovery(
                    f"[DISCOVERY] Current progress: {self.stats['total_generated']} algorithms generated"
                )
                raise
                ...
                if consecutive_errors >= self.max_consecutive_errors:
                    ...
                    try:
                        ...
                        try:
                            emergency_dir = (
                                Path(self.config.get("save_path", "./results"))
                                / f"{self.name}_emergency_backup"
                            )
                            ...
                            for i, algo in enumerate(self.history.algorithms):
                                algo_file = (
                                    emergency_dir
                                    / f"algorithm_{i}_{algo.get('id', 'unknown')}.json"
                                )
                                algo_file.write_text(
                                    json.dumps(self._sanitize_for_json(algo), indent=2)
                                )
                            ...
                            stats_file.write_text(
                                json.dumps(
                                    self._sanitize_for_json(self.stats), indent=2
                                )
                            )

                            self.log.warning(
                                f"Emergency backup saved to: {emergency_dir}"
                            )
                        except Exception as e:
                    ...
                    if checkpoint_saved:
                        self.log.discovery(
                            f"[DISCOVERY] Too many errors! Progress checkpoint saved."
                        )
                        self.log.discovery(
                            f"[DISCOVERY] To resume, run with the same config and name: '{self.name}'"
                        )
                        ...
                        self.log.discovery(
                            f"[DISCOVERY] Too many errors! WARNING: Checkpoint save failed."
                        )
                        self.log.discovery(
                            f"[DISCOVERY] Check logs for emergency backup location or start from beginning."
                        )

                    raise GenerationError(
                        f"Discovery loop failed after {consecutive_errors} consecutive errors"
                    )
        ...
        if self.verbose:
            ...
            self.log.discovery(
                f"Generated {summary['statistics']['total_generated']} algorithms"
            )
            self.log.discovery(
                f"Success rate: {summary['statistics']['success_rate']:.1%}"
            )
            self.log.discovery(
                f"Duration: {summary['statistics']['duration_minutes']:.1f} minutes"
            )
